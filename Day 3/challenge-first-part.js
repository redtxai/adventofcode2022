...
for (let i = 0; i < rucksacks.length ; i++) {
  const repeatedItemsRucksacks = []
  const firstCompartment =  rucksacks[i].substring(0, rucksacks[i].length / 2)
  const secondCompartment =  rucksacks[i].substring(rucksacks[i].length / 2)
  const itemsDictionary = {}

  for (let j = 0; j < firstCompartment.length ; j++) {
    itemsDictionary[firstCompartment[j]] = true
  }

  for (let j = 0; j < secondCompartment.length ; j++) {
    if (itemsDictionary[secondCompartment[j]] && !repeatedItemsRucksacks.find(val => secondCompartment[j])) {
      repeatedItemsRucksacks.push(secondCompartment[j])
    }
  }

  repeatedItems.push(...repeatedItemsRucksacks)
}
