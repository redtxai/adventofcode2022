...
for (let round = 0; round < 20; round++) {
  for (let monkeyInd = 0; monkeyInd < Monkeys.length; monkeyInd++) {
    ...
    for (let itemIndex = 0; itemIndex < monkey.items.length; itemIndex++) {
      ...
      const bored = Math.floor(worryLevel / 3)
      const checkDivisible = bored % monkey.test

      if (checkDivisible === 0) {
        Monkeys[monkey.true].items.push(bored)
      } else {
        Monkeys[monkey.false].items.push(bored)
      }
    }
...
console.log("first part answer:", first * second)
